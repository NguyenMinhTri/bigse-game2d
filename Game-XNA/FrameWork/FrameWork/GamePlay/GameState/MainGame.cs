...
namespace FrameWork.GamePlay.GameState
{
    class MainGame : iState
    {
        ...
        public MainGame(iPlay _iPlay, string MapName, Game game)
            : base(_iPlay, game)
        {
        ...
        public override void Init()
        {
            ...
            ViewRect = new Rectangle(0, 0, Game.GraphicsDevice.Viewport.Width, Game.GraphicsDevice.Viewport.Height);
            Cam2D = new Camera2D(Game, new Rectangle(-512, 0, 2048, 800), ViewRect);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                m_iRunning = false;
                m_Menu.iBackOld = Audio.Instance().BackGround;
            }
            ...
            if (mario.POSITION.X > building.POSITION.X + 250)
            {
                ...
                if (timeUpdate2 > 3000)
                {
                    ...
                    m_Win.RATE = Score.Instance().AddScore(score);
                    m_iRunning1 = false;

                    bool iEnd = true;
                    DirectoryInfo Dir = new DirectoryInfo("Content/Map/");
                    string name = "Level" + (Level + 1).ToString();
                    if (Level==0)
                    {
                        iEnd = false;
                    }
        ...
        public override void Render(GameTime gameTime, SpriteBatch _SpriteBatch)
        {
            ...
            for (int i = 0; i < CameraContent.Count; i++)
            {
                if(Cam2D.ViewRect.Intersects(CameraContent[i].RECT))
                    CameraContent[i].Render(_SpriteBatch);
            }
            ...
            if (Boss.POSITION.X - mario.POSITION.X <= Game.GraphicsDevice.Viewport.Width && Boss.STATUS != MyStatus.DEATH)
            {
